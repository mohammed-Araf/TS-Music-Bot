...
import ts3musicbot.util.EpisodeList
...
class Spotify(private val botSettings: BotSettings) : Service(ServiceType.SPOTIFY) {
    ...
    override suspend fun fetchAlbumTracks(
        ...
        suspend fun parseAlbumData(albumData: JSONObject) {
            val trackItemsLength =
                albumData.getJSONObject("tracks").getInt("total").let {
                    if (limit != 0 && it > limit) limit else it
                }
            val albumName =
                if (albumData.getString("album_type") == "single") {
    ...
    override suspend fun fetchTrack(trackLink: Link): Track {
        ...
        suspend fun parseData(trackData: JSONObject): Track {
            ...
            val albumArtists =
                Artists(
                    trackData.getJSONObject("album").getJSONArray("artists").map {
                        it as JSONObject
                        Artist(
                            Name(it.getString("name")),
                            Link(it.getJSONObject("external_urls").getString("spotify")),
            ...
            val artists =
                Artists(
                    trackData.getJSONArray("artists").map {
                        it as JSONObject
                        Artist(
                            Name(it.getString("name")),
                            Link(it.getJSONObject("external_urls").getString("spotify")),
    ...
    override suspend fun fetchArtist(
        ...
        suspend fun parseData(
            ...
            for (track in topTracksList) {
                ...
                val artists =
                    Artists(
                        track.getJSONArray("artists").mapTo(list) {
                            it as JSONObject
                            Artist(
                                Name(it.getString("name")),
                                Link(it.getJSONObject("external_urls").getString("spotify")),
                                            ...
                                            User(
                                                ...
                                                Link(
                                                    it.getJSONObject("owner").getJSONObject("external_urls")
                                                        .getString("spotify"),
                                                    it.getJSONObject("owner").getString("id"),
                                                ),
                                            ),
