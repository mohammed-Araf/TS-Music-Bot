...
class ChatReader(
    ...
    private fun isPlainSearchQuery(args: String): Boolean {
        val trimmed = args.trim()
        if (trimmed.isEmpty()) return false
        if (trimmed.contains("://") || trimmed.startsWith("spotify:")) return false
        
        val words = trimmed.split("\\s+".toRegex())
        if (words.size >= 2) {
            val firstWord = words[0].lowercase()
            val secondWord = words[1].lowercase()
            val validProviders = listOf("sp", "spotify", "s", "yt", "youtube", "y", "sc", "soundcloud", "bc", "bandcamp")
            val validTypes = listOf("track", "album", "playlist", "artist", "show", "episode", "video", "playlist", "channel")
            if (firstWord in validProviders && secondWord in validTypes) {
                return false
            }
        }
        return true
    ...
    private fun preprocessCommand(commandString: String): String {
        ...
        if (cmd.startsWith("${pfx}p ") || cmd.startsWith("${pfx}play ")) {
            ...
            if (args.isNotEmpty()) {
                val targetCmd = if (songQueue.getState() != SongQueue.State.QUEUE_STOPPED) queueAddCmd else queuePlaynowCmd
                val finalArgs = if (isPlainSearchQuery(args)) "sp track $args" else args
                cmd = "$targetCmd $finalArgs"
            }
        } else if (cmd.startsWith("${pfx}ps ")) {
            cmd = "$queueAddCmd sp track " + cmd.substring(pfx.length + 3)
        } else if (cmd.startsWith("${pfx}p s ")) {
            cmd = "$queueAddCmd sp track " + cmd.substring(pfx.length + 4)
        } else if (cmd.startsWith("${pfx}py ")) {
                    ...
                    if (cmdList.commandList.any { commandString.startsWith(it.value) } || commandString.startsWith("%help")) {
                        ...
                        when {
                            ...
                            commandString.contains(
                                (
                                    "^(${cmdList.commandList["queue-add"]}|${cmdList.commandList["queue-playnext"]})" +
                                        ...
                                        "((sp|spotify|s|yt|youtube|y|sc|soundcloud|bc|bandcamp)\\s+" +
                                ...
                                if (commandString.contains(
                                        "\\s+(sp|spotify|s|yt|youtube|y|sc|soundcloud|bc|bandcamp)\\s+\\w+\\s+.+".toRegex(),
                                    )
                                ) {
                                    when {
                                        commandString.contains("\\s+(sp|spotify|s)\\s+".toRegex()) ->
                                            spotify
                                                .search(
                                                    SearchType(
                                                        commandString
                                                            .split("\\s+(sp|spotify|s)\\s+".toRegex())
                                                            .last()
                                                            .substringBefore(" "),
                                                    ),
                                                    SearchQuery(
                                                        commandString.split("\\s+(sp|spotify|s)\\s+\\w+\\s+".toRegex()).last(),
                                                    ),
                                                    1,
                                                ).results
                                                .let {
                                        ...
                                        commandString.contains("\\s+(yt|youtube|y)\\s+".toRegex()) ->
                                            youTube
                                                .search(
                                                    SearchType(
                                                        commandString
                                                            .split("\\s+(yt|youtube|y)\\s+".toRegex())
                                                            .last()
                                                            .substringBefore(" "),
                                                    ),
                                                    SearchQuery(
                                                        commandString.split("\\s+(yt|youtube|y)\\s+\\w+\\s+".toRegex()).last(),
                                                    ),
                                                    1,
                                                ).results
                                                .let {
                                ...
                                if (commandSuccessful.isEmpty()) {
                                    val msg = "No tracks found or search query failed."
                                    printToChat(listOf(msg))
                                    commandListener.onCommandExecuted(commandString, msg, null)
                                    commandJob.complete()
                            ...
                            commandString.contains(
                                (
                                    "^${cmdList.commandList["queue-delete"]}((\\s+(-a|--all|-A|--all-artist-tracks|-f|--first))?" +
                                        "\\s+((sp|spotify|s|yt|youtube|y|sc|soundcloud|bc|bandcamp)\\s+\\w+\\s+|((\\[URL])?" +
                                ...
                                if (songQueue.getQueue().isNotEmpty()) {
                                    ...
                                    val services = listOf("sp", "spotify", "s", "yt", "youtube", "y", "sc", "soundcloud", "bc", "bandcamp")
                                    val links =
                                        when {
                                            commandString.contains(
                                                (
                            ...
                            commandString.contains("^${cmdList.commandList["volume"]}(\\s+[+-]?\\d+)?$".toRegex()) -> {
                                ...
                                if (args.isEmpty()) {
                                    // Get current volume
                                    val spVol = playerctl(botSettings.spotifyPlayer, "volume").outputText.trim()
                                    val mpvVol = playerctl("mpv", "volume").outputText.trim()
                                    val message = StringBuilder("Current volumes:")
                                    if (spVol.isNotEmpty()) message.append("\nSpotify: $spVol%")
                                    if (mpvVol.isNotEmpty()) message.append("\nYouTube/SoundCloud: $mpvVol%")
                                    if (spVol.isEmpty() && mpvVol.isEmpty()) {
                                        message.append("\nSpotify (Config): ${botSettings.ytVolume}%")
                                        message.append("\nYouTube (Config): ${botSettings.ytVolume}%")
                                    }
    ...
    private suspend fun chatUpdated(
        ...
        try {
            ...
            println("Exception caught in chatUpdated loop: ${t.message}")
            t.printStackTrace()
        }
    }
